...
import VendasGraficos from "../../components/Gráficos/VendasGraficos"; 
...
export default function SaleListView() {
  ...
  const [showMetrics, setShowMetrics] = useState(false); // Estado para alternar entre lista e gráfico
  ...
  return (
    <section class="flex h-full w-full flex-col gap-2 overflow-y-scroll bg-white-0 p-4">
      <header class="flex items-center gap-2">
        ...
        <h1 class="font-bold">{showMetrics ? "Gráficos de Vendas" : "Lista de Vendas"}</h1>
      </header>
      <div class="flex justify-end gap-1">
        <IconButton iconName="Graph" onClick={() => setShowMetrics(!showMetrics)}>
          {showMetrics ? "Voltar à Lista" : "Métricas"}
        </IconButton>
        {!showMetrics && (
          <>
            <IconButton
